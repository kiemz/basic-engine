...
private:
    ...
    void loadModel(std::string const& name)
    {
        ...
        auto real_path = path + "source/objects/backpack/" + name;
        const aiScene* scene = importer.ReadFile(real_path, aiProcess_Triangulate | aiProcess_GenSmoothNormals | aiProcess_FlipUVs | aiProcess_CalcTangentSpace);
        // check for errors
        ...
        directory = real_path.substr(0, real_path.find_last_of('/'));
